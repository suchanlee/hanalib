...
const openLibraryPayload = {
  ...
  publishers: ['HarperVia'],
  publish_date: '2020',
  languages: [{ key: '/languages/eng' }],
  isbn_13: ['9788936434267'],
};
...
function providerFetch(
  ...
  return async (input, init) => {
    ...
    if (url.hostname === 'openlibrary.org') {
      if (options.failOpenLibrary)
        return Response.json({ error: 'down' }, { status: 503 });
      if (url.pathname.startsWith('/authors/'))
        return Response.json({ name: 'Won-pyung Sohn' });
      if (url.pathname === '/search.json')
        return Response.json(openLibrarySearchPayload);
...
void test('fetches providers in parallel and stitches fields deterministically for Korean', async () => {
  ...
  assert.equal(
    ...
    'https://books.google.test/almond.jpg',
  );
...
void test('falls back to Open Library search when its exact-edition endpoint fails', async () => {
  ...
  assert.equal(result.metadata?.title, 'Almond');
  assert.deepEqual(result.metadata?.authors, ['Won-pyung Sohn']);
  assert.equal(result.providerStatus['open-library'], 'partial');
  assert.ok(
    result.diagnostics.some(
      (e) =>
        e.stage === 'edition' && e.outcome === 'failed' && e.attempts === 2,
    ),
  );
});
...
void test('selects the most edition-specific exact title when Korean authorities are unavailable', async () => {
  const google = {
    items: [
      {
        volumeInfo: {
          title: 'Talk to me in Korean workbook',
          authors: ['TalkToMeInKorean'],
          publishedDate: '2018',
          language: 'en',
          industryIdentifiers: [
            { type: 'ISBN_13', identifier: '9791186701140' },
          ],
        },
      },
    ],
  ...
  const fetchImpl: FetchLike = async (input) => {
    ...
    if (url.pathname.startsWith('/authors/'))
      return Response.json({ name: 'TalkToMeInKorean' });
...
void test('reports a provider outage without substituting a different edition', async () => {
  const result = await resolveBookMetadata(
    ...
    'ko',
    {},
    { fetchImpl: providerFetch({ failOpenLibrary: true }) },
  );
        ...
        docs: [
          {
            TITLE: '작별하지 않는다',
            AUTHOR: '한강',
            EA_ISBN: '9788954682152',
            PUBLISHER: '문학동네',
            PUBLISH_PREDATE: '20210909',
          },
        ],
      });
    }
        ...
        items: [
          {
            ...
            image: 'https://naver.test/cover.jpg',
            description: '한국어 소개',
          },
        ],
      });
...
void test('allows fixtures only when explicitly enabled outside production', () => {
  assert.equal(
    fixtureLookupAllowed(false, {
      ...
      ISBN_FIXTURES_ENABLED: undefined,
    }),
    false,
  );
  assert.equal(
    fixtureLookupAllowed(true, {
      NODE_ENV: 'development',
      ISBN_FIXTURES_ENABLED: undefined,
    }),
    true,
